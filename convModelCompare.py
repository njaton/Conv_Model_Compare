...
from keras import layers, models  # For CNN
...
# Get the names and locations of all the files, add your folder here
start_folder = "/Users/nickjaton/Documents/datasets/sp-society-camera-model-identification/"
...
camera_list = ["HTC-1-M7", "iPhone-4s", "iPhone-6", "LG-Nexus-5x", "Motorola-Droid-Maxx", "Motorola-Nexus-6",
...
np.save("x_train.npy", x_train) # Do not commit this file

# Get the testing values
x_test = np.array([read_image(filepath) for filepath in list_test])
np.save("x_test.npy", x_test)  # Do not commit this file
...
def get_model():
    ...
    layers = Convolution2D(32, kernel_size=2, activation=activations.relu, padding="same")(layers)
...
# Get the fit history
history = model.fit(x_train_sub, y_train_sub, epochs=10, validation_data=(x_test_sub, y_test_sub))

# Finish collecting results from CNN model.
